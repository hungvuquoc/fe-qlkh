...
import useStore, { SUPPLIER } from '~/store/hooks';
import { useEffect, useState } from 'react';
import SupplierActions from '~/store/actions/SupplierActions';
import SupplierApi from '~/store/api/SupplierApi';
import DiaglogCreateAndUpdate from '~/pages/supplier/DialogCreateAndUpdate';

const cx = classNames.bind(IndexStyle);

function Supplier() {
  const [showDialogCreate, setShowDialogCreate] = useState(false);
  const [state, dispatch] = useStore(SUPPLIER);
  ...
  const fetchData = async (params = { pageIndex: pageIndex, pageSize: pageSize, keyword: '' }) => {
    try {
      const response = await SupplierApi.search(params);
      dispatch(SupplierActions.search(response));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleDeleteById = async (item) => {
    if (window.confirm(`Bạn muốn xóa nhà cung cấp này không [${item.id}-${item.name}]`) === false) return;
    try {
      await SupplierApi.deleteById(item.id);
      fetchData();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      <div className={cx('container')}>
        ...
        {showDialogCreate && <DiaglogCreateAndUpdate id={supplierId} handleClose={handleCloseDialogCreate} />}
        <div className={cx('view')}>
          <div className={cx('view-table')}>
            <TableV3 columns={configTableV3} datas={state.suppliers} cx={cx} emptyMessage={'Không có dữ liệu.'} />
          </div>
