...
import style from './EditProductSupplier.module.scss';
import ProductSupplierApi from '~/store/api/SupplierApi';
...
function EditProductSupplier({ dataOld = [], handleClose, handleConfirm }) {
  ...
  const fetchData = async (condition = { pageIndex: 1, pageSize: 9999, keyword: '' }) => {
    try {
      const response = await ProductSupplierApi.search(condition);
      const supplierResp = response?.content.map((obj1) => ({
        ...obj1,
  ...
  const handleChecked = (rowData, event) => {
    if (event.target.checked) {
      ...
      setDataNew(dataNew.filter((supplier) => supplier?.id !== rowData.id));
    }
    const supplierNews = [...suppliers];
    supplierNews.map((supplier) => {
      if (supplier.id === rowData.id) {
        supplier['checked'] = event.target.checked;
      }
    });

    setSuppliers(supplierNews);
  };
          ...
          <div className={cx('modal-content')}>
            ...
            <div className={cx('modal-body')}>
              ...
              <div className={cx('body_table')}>
                <TableV3 columns={configTableV3} datas={suppliers} cx={cx} emptyMessage={'Không có dữ liệu.'} />
              </div>
            </div>
