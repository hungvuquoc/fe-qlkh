...
import useStore, { WH_EXPORT } from '~/store/hooks';
import { useState, useEffect } from 'react';
import WhExportApi from '~/store/api/wh-transactions/WhExportApi';
import WhExportActions from '~/store/actions/wh-transactions/WhExportAction';
import TableV3, {
...
function WhExport() {
  const [state, dispatch] = useStore(WH_EXPORT);
  const [whExportId, setWhExportId] = useState(null);
  ...
  const fetchData = async (data = { pageIndex: pageIndex, pageSize: pageSize, keyword: '' }) => {
    try {
      ...
      const response = await WhExportApi.search(params);
      dispatch(WhExportActions.search(response));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleDeleteById = async (id) => {
    ...
    try {
      await WhExportApi.deleteById(id);
      fetchData();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      <div className={cx('container')}>
        ...
        {showDialogCreate && <DialogCreateAndUpdate id={whExportId} handleClose={handleCloseDialogCreate} />}
        <div className={cx('view')}>
          <div className={cx('view-table')}>
            <TableV3 columns={configTableV3} datas={state?.whExports} cx={cx} emptyMessage={'Không có dữ liệu.'} />
          </div>
