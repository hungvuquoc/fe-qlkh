...
import useStore, { ROLE } from '~/store/hooks';
import TableV3, {
...
import RoleActions from '~/store/actions/RoleActions';

const cx = classNames.bind(Style);

function Role() {
  const [state, dispatch] = useStore(ROLE);
  ...
  const fetchData = async (keyword) => {
    try {
      const response = await RoleApi.search({ pageIndex: pageIndex, pageSize: pageSize, keyword: keyword });
      dispatch(RoleActions.search(response));
    } catch (error) {
  ...
  return (
    <>
      {showDialog && <DialogCreateAndUpdate id={RoleId} handleCloseDialog={handleCloseDialog} />}
      <div className={cx('container')}>
        ...
        <div className={cx('view')}>
          <div className={cx('view-table')}>
            <TableV3 columns={configTableV3} datas={state.roles} cx={cx} emptyMessage={'Không có dữ liệu.'} />
          </div>
