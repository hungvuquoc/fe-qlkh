...
import useStore, { PRODUCT_GROUP } from '~/store/hooks';
import { useEffect, useState } from 'react';
import ProductGroupActions from '~/store/actions/ProductGroupActions';
import ProductGroupApi from '~/store/api/ProductGroupApi';
import DiaglogCreateAndUpdate from '~/pages/product-group/DialogCreateAndUpdate';

const cx = classNames.bind(IndexStyle);

function ProductGroup() {
  const [showDialogCreate, setShowDialogCreate] = useState(false);
  const [state, dispatch] = useStore(PRODUCT_GROUP);
  ...
  const fetchData = async (params = { pageIndex: pageIndex, pageSize: pageSize, keyword: '' }) => {
    try {
      const response = await ProductGroupApi.search(params);
      dispatch(ProductGroupActions.search(response));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleDeleteById = async (item) => {
    ...
    try {
      await ProductGroupApi.deleteById(item.id);
      fetchData();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      <div className={cx('container')}>
        ...
        {showDialogCreate && <DiaglogCreateAndUpdate id={productGroupId} handleClose={handleCloseDialogCreate} />}
        <div className={cx('view')}>
          <div className={cx('view-table')}>
            <TableV3 columns={configTableV3} datas={state.productGroups} cx={cx} emptyMessage={'Không có dữ liệu.'} />
          </div>
